...
fn test_key_storage_integration() {
    ...
    let ciphertext = cipher
        .encrypt(&loaded_public, plaintext)
        .expect("Encryption with loaded key failed");
    let decrypted = cipher
        .decrypt(&loaded_private, &ciphertext)
...
fn test_config_validation_integration() {
    ...
    let invalid_result = Config::builder().memory_limit_mb(0).build();
    assert!(
        invalid_result.is_err(),
        "Invalid config should fail to build"
    );
}
...
fn test_error_handling_integration() {
    ...
    let result = file_cipher.encrypt_file(non_existent, &output, keypair.public_key(), None);
    assert!(
        result.is_err(),
        "Encryption of non-existent file should fail"
    );
}
    ...
    for i in 0..5 {
        ...
        let handle = thread::spawn(move || {
            ...
            let ciphertext = cipher_clone
                .encrypt(keypair_clone.public_key(), plaintext_bytes)
