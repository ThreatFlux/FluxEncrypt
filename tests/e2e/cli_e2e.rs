...
fn test_encrypt_with_different_cipher_suites() {
    ...
    for cipher_suite in &cipher_suites {
        ...
        assert!(
            ...
            "Encryption with {} should succeed",
            cipher_suite
        );
        ...
        assert!(
            output.status.success(),
            "Decryption with {} should succeed",
            cipher_suite
        );
...
fn test_info_command() {
    ...
    assert!(
        stdout.contains("RSA") || stdout.contains("rsa"),
        "Should indicate RSA key type"
    );
}
...
fn test_verify_command() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("valid") || stdout.contains("Valid") || stdout.contains("OK"),
...
fn test_error_handling() {
    ...
    let output = run_cli(&[
        ...
        "--output",
        temp_dir.path().join("output.enc").to_str().unwrap(),
    ]);
